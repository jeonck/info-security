...
flowchart LR
    A["대칭키 유출 위협"] -- "수신자 공개키로 키 암호화" --> B["안전한 키 전달 환경"]
    style A fill:#f9f9f9,stroke:#333,stroke-width:1px
    style B fill:#e1f5fe,stroke:#01579b,stroke-width:1px
```

**정의**: 대량의 데이터를 빠르게 처리하기 위한 대칭키와, 그 키를 안전하게 분배하기 위한 **비대칭키**(공개키)의 장점을 결합한 하이브리드 암호 시스템의 핵심 메커니즘

**특징**:  
 (기밀성 보장) 수신자의 공개키로 암호화된 세션키를 통해서만 데이터를 복구할 수 있음  
 (키 관리 효율성) 일회성 세션키를 사용하여 키 노출 위험을 줄이고 관리 편의성을 높임  
 (하이브리드 구조) 대칭키의 속도와 비대칭키의 안전한 키 분배 장점을 결합한 형태임  
