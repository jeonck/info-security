...
flowchart LR
    A["인증/무결성/기밀성 개별"] -- "전자서명 + 전자봉투 결합" --> B["전 구간 통합 보안 모델"]
    style A fill:#f9f9f9,stroke:#333,stroke-width:1px
    style B fill:#e1f5fe,stroke:#01579b,stroke-width:1px
```

**정의**: 전자서명의 인증·무결성·부인방지 기능과 전자봉투의 기밀성 기능을 결합하여 전송 전 구간의 보안성을 확보하는 메커니즘

**필요성**:  
 (통합 보안 전 구간 확보) 인증, 무결성, 부인 방지, 기밀성을 동시에 확보하여 전 구간 보안을 완성함  
 (송신자 사칭 방어) 전자서명을 통해 송신자 신원을 검증함으로써 타인의 사칭을 원천 차단함  
 (재전송 공격 방지) 일회성 세션키와 서명 결합을 통해 메시지 재사용 및 변조를 방어함  
